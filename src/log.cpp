...
void Logger::println(const String str) {
    ...
    if (!FLAGS[TIME_INITIALIZED]) _serial_history_timestamps[_serial_history_idx] = "START+" + String(millis()/1000);
    else _serial_history_timestamps[_serial_history_idx] = STRINGS[TIMESTAMP];
    ...
    if (_serial_history_length < LOG_MAX_LENGTH) _serial_history_length++;
...
String Logger::get_line_i_timestamp_from_serial_history(int i) {
    ...
    if (_serial_history_length < LOG_MAX_LENGTH && _serial_history_idx - (i + 1) < 0) return "";
