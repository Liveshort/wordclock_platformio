...
void LEDController::overlay_AP_active(bool ap_active) {   
    ...
    if (!ap_active) direction = -abs(direction);
    else direction = abs(direction);

    // Breathing animation for the wifi LED
    if (ANIMATION_STATES[OVERLAY_AP_ACTIVE] < step && direction < 0) ANIMATION_STATES[OVERLAY_AP_ACTIVE] = 0;
    else if (ANIMATION_STATES[OVERLAY_AP_ACTIVE] > 255-step && direction > 0) ANIMATION_STATES[OVERLAY_AP_ACTIVE] = 255;
    else ANIMATION_STATES[OVERLAY_AP_ACTIVE] += direction;

    // leds[WIFI_LED_INDEX] = blend(leds[WIFI_LED_INDEX], CRGB(0, 0, ANIMATION_STATES[OVERLAY_AP_ACTIVE]), ANIMATION_STATES[OVERLAY_AP_ACTIVE]);
    leds_logical[WIFI_LED_INDEX] = blend(leds_logical[WIFI_LED_INDEX], CRGB(0, 0, ANIMATION_STATES[OVERLAY_AP_ACTIVE]), ANIMATION_STATES[OVERLAY_AP_ACTIVE]);
}
...
void LEDController::waiting_for_wifi_breathing_animation() {
    ...
    for (int i = 0; i < NUM_LEDS_LOGICAL; i++) leds_logical[i] = CRGB::Black;
    
    // Breathing animation for the wifi LED
    ANIMATION_STATES[WIFI_BREATHING] += direction;
    if (ANIMATION_STATES[WIFI_BREATHING] < step || ANIMATION_STATES[WIFI_BREATHING] > 255-step) direction = -direction;
    // leds[WIFI_LED_INDEX] = CRGB(ANIMATION_STATES[WIFI_BREATHING], ANIMATION_STATES[WIFI_BREATHING], ANIMATION_STATES[WIFI_BREATHING]);
    leds_logical[WIFI_LED_INDEX] = CRGB(ANIMATION_STATES[WIFI_BREATHING], ANIMATION_STATES[WIFI_BREATHING], ANIMATION_STATES[WIFI_BREATHING]);
}
...
void LEDController::wifi_connected_blink() {
    ...
    if (ANIMATION_STATES[WIFI_CONNECTED_BLINK] >= 128) FLAGS[TRANSITIONING] = false;
    // if ((ANIMATION_STATES[WIFI_CONNECTED_BLINK] / 32) % 2 == 0) leds[WIFI_LED_INDEX] = CRGB::Black;
    if ((ANIMATION_STATES[WIFI_CONNECTED_BLINK] / 32) % 2 == 0) leds_logical[WIFI_LED_INDEX] = CRGB::Black;
    // else leds[WIFI_LED_INDEX] = CRGB::White;
    else leds_logical[WIFI_LED_INDEX] = CRGB::White;
}
...
void LEDController::waiting_for_time_breathing_animation() {
    ...
    if (ANIMATION_STATES[TIME_SYNC_BREATHING] < step || ANIMATION_STATES[TIME_SYNC_BREATHING] > 255-step) direction = -direction;
    
    // leds[3] = CRGB(ANIMATION_STATES[TIME_SYNC_BREATHING], ANIMATION_STATES[TIME_SYNC_BREATHING], ANIMATION_STATES[TIME_SYNC_BREATHING]);
    for (int i = 0; i < 4; ++i)
            leds_logical[W2LL[TIJD][i]] = CRGB(ANIMATION_STATES[TIME_SYNC_BREATHING], ANIMATION_STATES[TIME_SYNC_BREATHING], ANIMATION_STATES[TIME_SYNC_BREATHING]);
}
...
void LEDController::time_synced_blink() {
    ...
    // Blink the time LEDs
    if (ANIMATION_STATES[TIME_SYNCED_BLINK] < 128) ANIMATION_STATES[TIME_SYNCED_BLINK] += direction;
    if (ANIMATION_STATES[TIME_SYNCED_BLINK] >= 128 && !FLAGS[FADING_OUT] && !FLAGS[FADING_IN]) FLAGS[TRANSITIONING] = false;
    // if (((ANIMATION_STATES[TIME_SYNCED_BLINK] / 32) % 2 == 0) && ANIMATION_STATES[TIME_SYNCED_BLINK] < 128) leds[3] = CRGB::Black;
    if (((ANIMATION_STATES[TIME_SYNCED_BLINK] / 32) % 2 == 0) && ANIMATION_STATES[TIME_SYNCED_BLINK] < 128) for (int i = 0; i < 4; ++i) leds_logical[W2LL[TIJD][i]] = CRGB::Black;
    
    // else leds[3] = CRGB::White;
    else for (int i = 0; i < 4; ++i) leds_logical[W2LL[TIJD][i]] = CRGB::White;
    
    if (FLAGS[FADING_OUT]) fade_out();
    else if (FLAGS[FADING_IN]) fade_in();
}
...
void LEDController::update() {
    ...
    for (int i = 0; i < NUM_LEDS_PHYSICAL; i++) leds_physical[i] = CRGB::Black;
    for (int i = 0; i < NUM_LEDS_LOGICAL; i++) leds_physical[L2P[i]] = leds_logical[i];

    // Apply gamma correction based on average light level, this makes the brightness levels appear more linear to the human eye
    // for (int i = 0; i < NUM_LEDS; i++)
    //     leds[i].fadeToBlackBy(GAMMA_CORRECTION_PERCENTAGE[max(leds[i].r, max(leds[i].g, leds[i].b))]);
    for (int i = 0; i < NUM_LEDS_PHYSICAL; i++)
        leds_physical[i].fadeToBlackBy(GAMMA_CORRECTION_PERCENTAGE[max(leds_physical[i].r, max(leds_physical[i].g, leds_physical[i].b))]);
